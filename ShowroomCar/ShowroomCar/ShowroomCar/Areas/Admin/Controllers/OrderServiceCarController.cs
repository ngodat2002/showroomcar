...
{
    public class OrderServiceCarController : Controller
    {
        // GET: Admin/OrderServiceCar
        private ShowroomCarDbContext db = new ShowroomCarDbContext();
        // GET: Admin/User
        public ActionResult Index()
        {
            var list = db.Orders.Where(m => m.Status != 0 && m.Type == 2).OrderByDescending(m => m.ID).ToList();
            return View(list);
        }
        ...
        public ActionResult PrintOrder(int? id)
        {
            ...
            ViewBag.carOrder = db.Cars.Find(muser.CarID);
            return View("~/Areas/Admin/Views/OrderServiceCar/PrintOrder.cshtml", muser);
        }
        public ActionResult RegistrationOrder(int? id)
        {
            Order mOrder = db.Orders.Find(id);
            mOrder.Type = 3;
            mOrder.PaymentMethod = "Cash";
            mOrder.PaymentStatus = 0;
            mOrder.Status = 2;
            db.Entry(mOrder).State = EntityState.Modified;
            db.SaveChanges();
            Message.set_flash("Create order success", "success");
            return Redirect("~/admin/OrderServiceCar/index");
        }
    }
}